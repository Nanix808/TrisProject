<template>
    <div v-if="props.isOpen">
        <BasePopUP :name=name @close="close">
            ...
            <div class="user-box">
                <PasswordInput :label="'Подтвердите пароль'" @valid_value="set_password_confirm_valid_value">
                </PasswordInput>
            </div>
            <div class="button-box">
                <BaseButton :name="'Войти'"
                    :is-active="isEmailValid && isPasswordValid && isPasswordConfirmValid && passwordConfirmValue === passwordValue"
                    @click="userRegister">
                </BaseButton>
...
function userRegister() {
    store.dispatch('register', { email: emailValue.value, password: passwordValue.value });
}
